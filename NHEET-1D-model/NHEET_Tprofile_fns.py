...
def post_process_responses(pamb,const_input_vals,rxn_consts,geom_vals,Matrix,Iminmax,tminmax,Tminmax,n,dirResp,dirName):
    ...
    for j in range(1,len(t_Sc)):
        Rep = rhof[j,0]*Dp*V[j,0]/muf[j,0]
        Pr= muf[j,0]*cpf[j,0]/kf[j,0]
        Tfref=Tf[j,0]
        for i in range(0,N):
    ...
    cprint('\t Tf.out = %.5f K' % (Tf[iall[indx],-1]),'cyan')
