...
def Koekemoer_PressureLoss(Dp,Dpcorr,phi,eps,x,V0,rhof_mean,muf_mean):    
    ...
    ##Carman1937, see Allan2013
    #ReErg=rhof_mean*V0*Duse/(muf_mean*(1-eps))
    #delta_p = x*rhof_mean*V0**2*(1-eps)/(Duse*eps**3)*(180./ReErg+2.87/ReErg**0.1)
    return delta_p
...
def NusseltNumber(eps,Rep,Pr,Nu_method):
    ...
    elif Nu_method ==3: #Beek1962, see Ismail1999
        tmp = 3.22*Rep**(1./3.)*Pr**(1./3.)+0.117*Rep**(0.8)*Pr**(0.4)
    elif Nu_method ==4: #Handley and Heggs1968
        tmp = (2.+1.1*Rep**0.6*Pr**(1./3.))
        #tmp = 0.217*Rep**0.756
    return tmp
